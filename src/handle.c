...
void release_slot(pcache_volume *volume) {
    ...
    /* Validate that v points to a slot within s_vols */
    if (volume >= s_vols && volume < s_vols + s_vol_count)
        volume->in_use = false;
...
pcache_volume *volume_from_handle(pcache_handle handle) {
    ...
    bool   valid = idx < s_vol_count && s_vols[idx].in_use;
...
pcache_volume *volume_from_handle_for_close(pcache_handle handle) {
    ...
    size_t i     = (size_t)(handle - 1);
    bool   valid = i < s_vol_count && s_vols[i].in_use;
...
pcache_handle handle_of(const pcache_volume *volume) {
    return (pcache_handle)(volume - s_vols + 1);
}

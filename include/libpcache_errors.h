...
typedef enum pcache_close_error {
    PCACHE_CLOSE_OK             = PCACHE_OK,             /**< Operation succeeded. */
    PCACHE_CLOSE_INVALID_HANDLE = PCACHE_INVALID_HANDLE, /**< The handle is zero or does not refer to an open volume. */
    PCACHE_CLOSE_SQLITE_ERROR   = PCACHE_SQLITE_ERROR, /**< sqlite3_close reported an error; inspect @p sqlite_error. */
    PCACHE_CLOSE_IO_ERROR = PCACHE_IO_ERROR, /**< Closing the data file descriptor failed; inspect @p posix_error. */
} pcache_close_error;

/** Outcome of ::pcache_get_configuration. */
typedef enum pcache_get_configuration_error {
    PCACHE_GET_CONFIGURATION_OK = PCACHE_OK, /**< Operation succeeded. */
...
typedef enum pcache_check_page_error {
    PCACHE_CHECK_PAGE_OK = PCACHE_OK, /**< Lookup completed successfully; consult the return value. */
    PCACHE_CHECK_PAGE_INVALID_HANDLE =
        PCACHE_INVALID_HANDLE, /**< The handle is zero or does not refer to an open volume. */
    PCACHE_CHECK_PAGE_SQLITE_ERROR = PCACHE_SQLITE_ERROR, /**< A SQLite call failed; inspect @p sqlite_error. */
...
typedef enum pcache_preallocate_error {
    PCACHE_PREALLOCATE_OK = PCACHE_OK, /**< Preallocation completed. */
    PCACHE_PREALLOCATE_INVALID_HANDLE =
        PCACHE_INVALID_HANDLE,                         /**< The handle is zero or does not refer to an open volume. */
    PCACHE_PREALLOCATE_IO_ERROR     = PCACHE_IO_ERROR, /**< A data-file I/O call failed; inspect @p posix_error. */
    PCACHE_PREALLOCATE_SQLITE_ERROR = PCACHE_SQLITE_ERROR, /**< A SQLite call failed; inspect @p sqlite_error. */
